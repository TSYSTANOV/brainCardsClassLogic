...
class EditCategory{
    ...
    addListener(HTMLelement){
        HTMLelement.addEventListener('click',()=>{
            ...
            if(event.target.classList.contains('edit__save')){
                ...
                const title = app_root.querySelector('.edit__title').textContent.trim()
    ...
    renderCells(pairs){
        ...
        tr.innerHTML = `
            <td class="table__cell table__cell_one" contenteditable="true">${pairs ? pairs[0] : ''}</td>
            <td class="table__cell table__cell_two" contenteditable="true">${pairs ? pairs[1] : ''}</td>
        `
    ...
    mergeCells(HTMLelement){
        ...
        HTMLelement.querySelectorAll('tr').forEach((item)=>{
            const first = item.children[0].textContent.length !== 0 ? item.children[0].textContent : null
            const second = item.children[1].textContent !== 0 ? item.children[1].textContent : null
            if(first && second){
